...
void Laborator7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	material = glGetUniformLocation(shader->program, "material_kd");  // componenta difuza
	glUniform1f(material, materialKd);

	material = glGetUniformLocation(shader->program, "material_ks");  // componenta speculara
	glUniform1f(material, materialKs);

	material = glGetUniformLocation(shader->program, "object_color");
